    ...
    <header className="glass-panel-luxury px-6 md:px-8 py-4 md:py-4.5 sticky top-0 z-20 transition-all duration-300">
      <div className="flex flex-col 2xl:flex-row 2xl:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="flex flex-col gap-1.5 min-w-0">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {/* Cryptographic Security (JWT Authentication) Doctor Status Badge (Phase 4) */}
              <button
                onClick={onOpenAuthModal}
                className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold border shadow-xs transition-all cursor-pointer bg-slate-900/90 dark:bg-slate-900 hover:bg-slate-800 text-teal-300 border-teal-500/40 hover:border-teal-400 group"
                title={`Authenticated Physician: ${loggedInDoctor?.name || 'Dr. Sharma'}\nDepartment: ${loggedInDoctor?.department || 'Endocrinology & DFU'}\nCryptographic Token: HS256 HMAC-SHA256\nClick to switch doctor profile or re-authenticate.`}
              >
                <Lock className="w-3 h-3 text-teal-400 group-hover:scale-110 transition-transform" />
                <span className="font-extrabold">{loggedInDoctor?.name || 'Dr. Sharma'}</span>
                <span className="text-[9px] font-mono px-1.5 py-0.2 rounded bg-teal-950 text-teal-300 border border-teal-800/80">
                  HS256
                </span>
              </button>
            </div>
        ...
        <div className="flex items-center gap-3 flex-wrap justify-end shrink-0">
          ...
          {/* Phase 9: Autonomous Clinical RAG Scribe & Note Generator */}
          <button
            onClick={onOpenScribeModal}
            className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold text-cyan-800 dark:text-cyan-200 bg-cyan-50 dark:bg-cyan-950/40 hover:bg-cyan-100 dark:hover:bg-cyan-900/50 border border-cyan-300 dark:border-cyan-700/60 rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer shadow-xs group"
            title="Generate Autonomous IWGDF 2023 Clinical Scribe SOAP Note (Clinical RAG Engine)"
          >
            <BookOpen className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400 group-hover:scale-110 transition-transform" />
            <span className="font-extrabold">AI Scribe</span>
            <span className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
          </button>

          {/* Phase 10: Federated Learning Topology & Consortium Hub */}
          <button
            onClick={onOpenFederatedModal}
            className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold text-indigo-800 dark:text-indigo-200 bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 border border-indigo-300 dark:border-indigo-700/60 rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98] cursor-pointer shadow-xs group"
            title="Privacy-Preserving Multi-Center Federated AI Consortium (FedAvg + Differential Privacy)"
          >
            <Network className="w-3.5 h-3.5 text-indigo-600 dark:text-indigo-400 group-hover:scale-110 transition-transform" />
            <span className="font-extrabold">Federated AI</span>
