...
import ArchitectureHubModal from './ArchitectureHubModal'
import FhirExportModal from './FhirExportModal'
import ClinicalRAGScribeModal from './ClinicalRAGScribeModal'
...
const NAV_ITEMS = [
  ...
  { id: 'pillars', label: 'Architecture & AI Suite' },
...
export default function LandingPage({ onEnterWorkstation }) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero', 'pillars', 'technology', 'sinbad', 'evidence']
  ...
  return (
    <div className="min-h-screen bg-[#f4f8f5] dark:bg-[#0e120f] text-slate-800 dark:text-slate-100 selection:bg-[#aceba7]/30 selection:text-[#12464e] dark:selection:text-[#aceba7] overflow-x-hidden font-sans relative transition-colors duration-300">
      ...
      {/* 2.2 ENTERPRISE ARCHITECTURE & AUTONOMOUS CLINICAL AI HERO SUITE */}
      <section
        id="pillars"
        className="py-16 sm:py-20 relative z-10 max-w-[1440px] mx-auto px-6 sm:px-10 lg:px-14 border-t border-[#12464e]/10 dark:border-[#223229]"
      >
        {/* Section Header */}
        <div className="flex flex-col items-start max-w-3xl mb-12">
          <motion.div
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#aceba7]/15 dark:bg-[#aceba7]/10 border border-[#aceba7]/40 dark:border-[#aceba7]/30 text-[#12464e] dark:text-[#aceba7] text-xs font-bold uppercase tracking-wider mb-4"
          >
            <Boxes className="w-3.5 h-3.5" />
            <span>Enterprise Foundations & Autonomous Clinical AI Suite</span>
          </motion.div>
          ...
          <motion.h2
            ...
            className="text-3xl sm:text-4xl lg:text-5xl font-serif-luxury font-bold text-slate-900 dark:text-white tracking-tight leading-[1.2] mb-4"
          >
            Clinical Infrastructure & <br className="hidden sm:inline" />
            <span className="italic font-normal text-[#12464e] dark:text-[#aceba7]">
          ...
          >
            Engineered for mission-critical hospital networks — combining full-stack relational persistence, standardized HL7 FHIR exchange, ambient multi-modal clinical intelligence, and zero-PII federated training.
          </motion.p>
        </div>

        {/* 4 Interactive Enterprise Pillars Master Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {/* Pillar 1: 5-Pillar Enterprise Architecture Hub */}
          <motion.div
            ...
            whileHover={{ y: -6 }}
            className="spotlight-card glass-panel-luxury p-6 rounded-3xl border border-emerald-500/30 dark:border-emerald-500/20 flex flex-col justify-between shadow-lg hover:shadow-2xl hover:border-emerald-400/60 transition-all group relative overflow-hidden"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-2xl bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 flex items-center justify-center border border-emerald-500/30 shadow-xs group-hover:scale-110 transition-transform">
                  <Boxes className="w-6 h-6" />
                </div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-emerald-700 dark:text-emerald-300 bg-emerald-500/15 px-2.5 py-1 rounded-full border border-emerald-500/30 flex items-center gap-1.5">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                  5 Pillars
                </span>
              </div>

              <h3 className="text-lg font-serif-luxury font-bold text-slate-900 dark:text-white mb-2">
                5-Pillar Architecture Hub
              </h3>
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mb-4">
                Full-stack PostgreSQL persistence, sub-50ms WebSocket streaming, FHIR R4 interoperability, and ONNX containerized edge runtime.
              </p>

              <div className="space-y-2 mb-6 text-[11px] font-mono text-slate-700 dark:text-slate-300">
                ...
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                  <span>Real-Time WebSocket Queue</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                  <span>Docker Containerized Deploy</span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-[#0e120f] bg-[#aceba7] hover:bg-[#bbf0b7] transition-all flex items-center justify-center gap-2 shadow-sm shadow-[#aceba7]/20 cursor-pointer group-hover:scale-[1.02] border border-[#aceba7]/40"
            >
              <span>Explore 5 Pillars Hub</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </motion.div>

          {/* Pillar 2: HL7 FHIR R4 Standards Suite */}
          <motion.div
            initial={{ opacity: 0, y: 25 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            ...
            whileHover={{ y: -6 }}
            className="spotlight-card glass-panel-luxury p-6 rounded-3xl border border-teal-500/30 dark:border-teal-500/20 flex flex-col justify-between shadow-lg hover:shadow-2xl hover:border-teal-400/60 transition-all group relative overflow-hidden"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-2xl bg-teal-500/15 text-teal-700 dark:text-teal-300 flex items-center justify-center border border-teal-500/30 shadow-xs group-hover:scale-110 transition-transform">
                  <FileCode className="w-6 h-6" />
                </div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-teal-700 dark:text-teal-300 bg-teal-500/15 px-2.5 py-1 rounded-full border border-teal-500/30">
                  HL7 Standards
                </span>
              </div>

              <h3 className="text-lg font-serif-luxury font-bold text-slate-900 dark:text-white mb-2">
                FHIR R4 Document Suite
              </h3>
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mb-4">
                Complete EHR bi-directional integration standardizing wound observations and SINBAD metrics into validated FHIR Release 4 JSON bundles.
              </p>

              <div className="space-y-2 mb-6 text-[11px] font-mono text-slate-700 dark:text-slate-300">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-teal-500 shrink-0" />
                  <span>SNOMED-CT & LOINC Mapping</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-teal-500 shrink-0" />
                  <span>FHIR ClinicalImpression & Bundle</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-teal-500 shrink-0" />
                  <span>Epic & Cerner EHR Ready</span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-teal-700 hover:bg-teal-600 transition-all flex items-center justify-center gap-2 shadow-sm shadow-teal-700/20 cursor-pointer group-hover:scale-[1.02] border border-teal-600/40"
            >
              <span>Inspect FHIR R4 Bundle</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </motion.div>

          {/* Pillar 3: Autonomous Clinical RAG Scribe */}
          <motion.div
            initial={{ opacity: 0, y: 25 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            ...
            whileHover={{ y: -6 }}
            className="spotlight-card glass-panel-luxury p-6 rounded-3xl border border-cyan-500/30 dark:border-cyan-500/20 flex flex-col justify-between shadow-lg hover:shadow-2xl hover:border-cyan-400/60 transition-all group relative overflow-hidden"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-2xl bg-cyan-500/15 text-cyan-700 dark:text-cyan-300 flex items-center justify-center border border-cyan-500/30 shadow-xs group-hover:scale-110 transition-transform">
                  <BookOpen className="w-6 h-6" />
                </div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-cyan-700 dark:text-cyan-300 bg-cyan-500/15 px-2.5 py-1 rounded-full border border-cyan-500/30 flex items-center gap-1.5">
              ...
              <h3 className="text-lg font-serif-luxury font-bold text-slate-900 dark:text-white mb-2">
                Autonomous AI Scribe
              </h3>
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mb-4">
                Ambient clinical voice transcription and multi-modal RAG synthesizing comprehensive SOAP progress notes and debridement plans.
              </p>

              <div className="space-y-2 mb-6 text-[11px] font-mono text-slate-700 dark:text-slate-300">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-cyan-500 shrink-0" />
                  <span>Multi-Speaker Voice Dictation</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-cyan-500 shrink-0" />
                  <span>Automated SOAP Note Synthesis</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-cyan-500 shrink-0" />
                  <span>IWGDF Guideline Verification</span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-cyan-700 hover:bg-cyan-600 transition-all flex items-center justify-center gap-2 shadow-sm shadow-cyan-700/20 cursor-pointer group-hover:scale-[1.02] border border-cyan-600/40"
            >
              <span>Launch AI Clinical Scribe</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </motion.div>

          {/* Pillar 4: Privacy-Preserving Federated AI Consortium */}
          <motion.div
            initial={{ opacity: 0, y: 25 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            ...
            whileHover={{ y: -6 }}
            className="spotlight-card glass-panel-luxury p-6 rounded-3xl border border-indigo-500/30 dark:border-indigo-500/20 flex flex-col justify-between shadow-lg hover:shadow-2xl hover:border-indigo-400/60 transition-all group relative overflow-hidden"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                <div className="w-12 h-12 rounded-2xl bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 flex items-center justify-center border border-indigo-500/30 shadow-xs group-hover:scale-110 transition-transform">
                  <Network className="w-6 h-6" />
                </div>
                <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-indigo-700 dark:text-indigo-300 bg-indigo-500/15 px-2.5 py-1 rounded-full border border-indigo-500/30 flex items-center gap-1.5">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed mb-4">
                Multi-hospital decentralized AI model training with encrypted model weight aggregation and zero patient PII transmission.
              </p>

              <div className="space-y-2 mb-6 text-[11px] font-mono text-slate-700 dark:text-slate-300">
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                  <span>Decentralized Node Consensus</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                  <span>Differential Privacy (ε=0.85)</span>
                </div>
                <div className="flex items-center gap-2">
                  <CheckCircle2 className="w-3.5 h-3.5 text-indigo-500 shrink-0" />
                  <span>Encrypted Weights Aggregation</span>
                </div>
              </div>
            </div>

            <button
              onClick={() => setIsFederatedModalOpen(true)}
              className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-indigo-700 hover:bg-indigo-600 transition-all flex items-center justify-center gap-2 shadow-sm shadow-indigo-700/20 cursor-pointer group-hover:scale-[1.02] border border-indigo-600/40"
            >
              <span>Inspect Federated Network</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          ...
          >
            ...
            <div className="mt-7 pt-5 border-t border-slate-200/70 dark:border-white/5 flex items-center justify-between">
              <button
                onClick={handleOpenAuth}
                className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-500 hover:to-blue-500 transition-all shadow-md shadow-sky-500/20 flex items-center justify-center gap-2 cursor-pointer group/btn"
              >
          ...
          >
            ...
            <div className="mt-7 pt-5 border-t border-slate-200/70 dark:border-white/5 flex items-center justify-between">
              <a
                ...
                className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-[#12464e] dark:text-[#aceba7] bg-[#aceba7]/20 hover:bg-[#aceba7]/30 dark:bg-[#aceba7]/10 dark:hover:bg-[#aceba7]/20 border border-[#aceba7]/40 transition-all shadow-xs flex items-center justify-center gap-2 cursor-pointer group/btn"
              >
          ...
          >
            ...
            <div className="mt-7 pt-5 border-t border-slate-200/70 dark:border-white/5 flex items-center justify-between">
              <button
                onClick={handleOpenAuth}
                className="w-full py-2.5 px-4 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-rose-600 to-red-700 hover:from-rose-500 hover:to-red-600 transition-all shadow-md shadow-rose-600/20 flex items-center justify-center gap-2 cursor-pointer group/btn border border-rose-500/30"
              >
      ...
      {/* 9. ENTERPRISE ARCHITECTURE & CLINICAL AI SUITE MODALS */}
