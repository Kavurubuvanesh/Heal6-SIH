...
def load_model():
    ...
    possible_paths = [
        os.path.join(script_dir, "weights", "wound_detect_convnext.pth"),
        os.path.abspath(os.path.join(script_dir, "..", "..", "..", "..", "..", "backend", "app", "ml_engine", "weights", "wound_detect_convnext.pth")),
        os.path.abspath(os.path.join(script_dir, "..", "..", "..", "..", "backend", "app", "ml_engine", "weights", "wound_detect_convnext.pth")),
        os.path.abspath(os.path.join(script_dir, "..", "..", "weights", "wound_detect_convnext.pth"))
    ]
    weights_path = next((p for p in possible_paths if os.path.exists(p)), None)

    if not weights_path:
        raise FileNotFoundError(f"ConvNeXt model weights not found in local or root backend directories.")
