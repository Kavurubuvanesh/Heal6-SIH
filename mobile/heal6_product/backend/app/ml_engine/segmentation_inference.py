...
def load_segmentation_model():
    """Loads either the SOTA UNet++ (Multiclass) or Attention U-Net into memory."""
    ...
    possible_paths = [
        ...
        os.path.abspath(os.path.join(script_dir, "..", "..", "..", "..", "..", "backend", "models", "heal6_tissue_sota_best.pth")),
        os.path.abspath(os.path.join(script_dir, "..", "..", "..", "..", "..", "backend", "app", "ml_engine", "weights", "heal6_tissue_sota_best.pth")),
        os.path.abspath(os.path.join(script_dir, "..", "..", "..", "..", "..", "backend", "app", "ml_engine", "weights", "wound_segment_attention_unet.pth")),
        os.path.abspath(os.path.join(script_dir, "..", "..", "..", "..", "backend", "models", "heal6_tissue_sota_best.pth")),
        os.path.abspath(os.path.join(script_dir, "..", "..", "..", "..", "backend", "app", "ml_engine", "weights", "wound_segment_attention_unet.pth"))
    ]
    ...
    if target_path and "heal6_tissue_sota_best.pth" in target_path and HAS_SMP:
        print(f"⚡ [ML ENGINE] Loading SOTA UNet++ (EfficientNet-B4) from: {target_path}")
        ...
        print(f"⚡ [ML ENGINE] Loading Fallback Attention U-Net from: {target_path}")
        model = AttentionUNet(in_channels=3, out_channels=1)
        model.load_state_dict(torch.load(target_path, map_location=device, weights_only=True))
        _model_type = "attention_unet"
    else:
        print("⚠️ [ML ENGINE] No local weights found. Initializing architecture for evaluation.")
        model = AttentionUNet(in_channels=3, out_channels=1)
        _model_type = "attention_unet"
